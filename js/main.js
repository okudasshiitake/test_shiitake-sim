...
async function startBgm(index) {
    ...
    // 既存のオーディオがあり、一時停止中なら再開
    if (bgmAudio && index === undefined && bgmAudio.paused && bgmAudio.src) {
        try {
            bgmStarting = true;
            await bgmAudio.play();
            bgmPlaying = true;
    ...
    // HTML5 Audio の volume も設定（PCで効く）
    ...
    if (ctx) {
        try {
            ...
            // フォールバック: HTML5 volumeで対応（既に設定済み）
        }
    }
...
function init() {
    ...
    document.addEventListener('click', async function startBgmOnce() {
