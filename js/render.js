...
function renderLogActions(log, mature, season) {
    ...
    if ((log.stage === 'kariFuse' && log.fuseDays >= 45) || log.stage === 'honFuseReady') {
        ...
        // 本伏せボタンが初めて表示されるとき、チュートリアルをトリガー
        if (!mustWait && !gameState.honfuseTutorialShown && !gameState.firstActions.honFuse) {
            gameState.honfuseTutorialShown = true;
            saveState();
            // 少し遅延してチュートリアルを表示（レンダリング完了後）
            setTimeout(() => {
                if (typeof showHonfuseTutorial === 'function') {
                    showHonfuseTutorial();
                }
            }, 500);
        }
