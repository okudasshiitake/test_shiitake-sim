...
function advance(days) {
    ...
    for (let i = 0; i < days; i++) {
        ...
        // 人を雇用の毎月費用（人数×2500円）
        if (d.date === 1 && gameState.workerCount > 0) {
            const workerCost = gameState.workerCount * 2500;
            gameState.totalMoney -= workerCost;
            addEvent(`従業員${gameState.workerCount}人の給料 -${workerCost}円`, 'weather');
        }
        // 保険の毎月費用
        ...
        // クーラーの毎月費用
        if (d.date === 1 && gameState.ownedItems.includes('cooler')) {
            gameState.totalMoney -= 2000;
            addEvent(`クーラー電気代 -2000円`, 'weather');
        }
        ...
        if (gameState.totalMoney < 0) {
            ...
            if (gameState.ownedItems.includes('cooler')) {
                gameState.ownedItems = gameState.ownedItems.filter(id => id !== 'cooler');
                cancelItems.push('クーラー');
            }
...
function updateMaturingLog(log) {
    ...
    if (d.month >= 10) {
        ...
        // 初回の10月（収穫可能になった時）に栽培おさらいポップアップを表示
        if (!gameState.firstOctoberTutorialShown) {
            gameState.firstOctoberTutorialShown = true;
            setTimeout(() => {
                showFirstOctoberTutorial();
            }, 500);
        }
    }
}
...
function updateActiveLog(log, season, month, hasGreenhouse) {
    ...
    // クーラーがない場合のみ夏のイベント処理
    if (d.month >= 7 && d.month <= 9 && log.restDays === 0 && !hasCooler) {
        handleSummerEvents(log, d);
    }

    if (d.date === 1) log.didSummerTenchi = false;

    // 夏（7-9月）でクーラーがない場合は新規発生しないが、既存の椎茸は成長・収穫可能
    // クーラーがある場合は夏も通常通り栽培可能
    if (season.isSummer && !hasCooler) {
        // 既存のsprout/matureは成長を継続
